...
7. Context-only usage snapshots, when present: `src/data/openrouter-attribution.ts` and `src/data/ecosystem-signals.ts`. They are generated by `npm run sync:usage`; do not hand-edit their values. OpenRouter page totals and windowed metrics have separate dates and scopes. Homebrew, npm, VS Code Marketplace, and GitHub retain their native units, windows, populations, and explicit artifact mappings. Never add these signals together or use them as capability evidence or scoring inputs. Keep `/usage`, profile context, CSV export, freshness records, methodology, and tests aligned with any schema change.
...
- The dedicated usage refresh runs daily. It may update generated OpenRouter, Homebrew, npm, VS Code Marketplace, and GitHub context data, `/usage`, profile context, and the static CSV only; it must never reinterpret popularity as capability, quality, task success, or workflow fit.
