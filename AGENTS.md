...
7. Context-only OpenRouter attribution snapshots, when present: `src/data/openrouter-attribution.ts`. This file is generated by `npm run sync:openrouter` from canonical app pages and authenticated 1-day, 7-day, and 30-day coding datasets; do not hand-edit its values. Page totals and windowed metrics have separate dates and scopes. They are routing-channel observations, never capability evidence or scoring inputs. Keep `/usage`, profile context, CSV export, freshness records, methodology, and tests aligned with any schema change.
...
- The dedicated OpenRouter usage refresh runs daily. It may update generated attribution data, `/usage`, profile context, and the static CSV only; it must never reinterpret popularity as capability, quality, task success, or workflow fit.
- Every scheduled catalog run must execute `npm run sync:openrouter` before deciding that no data changed. The command may update only the context-only attribution dataset, must join ranking rows by stable app id rather than display name, and must fail closed on an identity, schema, source, or API error.
...
- OpenRouter attribution sync and stable app mapping: `scripts/sync-openrouter-attribution.mjs` and `scripts/lib/openrouter-sync.mjs`
- Channel-specific usage page and CSV: `src/app/usage/page.tsx`, `src/components/openrouter-usage-leaderboard.tsx`, and `src/app/usage.csv/route.ts`
