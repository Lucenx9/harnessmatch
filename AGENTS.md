...
7. Context-only usage snapshots, when present: `src/data/openrouter-attribution.ts` and `src/data/ecosystem-signals.ts`. They are generated by `npm run sync:usage`; do not hand-edit their values. OpenRouter page totals and windowed metrics have separate dates and scopes. Homebrew, npm, filtered stable GitHub release assets, VS Code Marketplace, Open VSX, JetBrains Marketplace, and GitHub repository interest retain their native units, windows, populations, and explicit artifact mappings. Release mappings must isolate the user-facing harness distribution from prereleases, checksums, source archives, SDKs, GUI packages, and unrelated monorepo artifacts. New mapped stable versions may create GPT-OSS editorial triage in `research/release-review-queue.json`, but that queue is never evidence and cannot propagate into claims or scores without first-party verification. Never add these signals together or use them as capability evidence or scoring inputs. Keep `/usage`, profile context, CSV export, freshness records, methodology, and tests aligned with any schema change.
...
- `.github/workflows/daily-usage-refresh.yml` is the zero-touch daily usage transaction. It may commit only `src/data/openrouter-attribution.ts`, `src/data/ecosystem-signals.ts`, and the repository-only `research/release-review-queue.json`; it must dispatch the quality gate for the published commit and verify the matching production deployment before reporting success. GPT-OSS may triage newly observed stable release notes into that queue, but its output is never evidence and cannot change capability, taxonomy, recommendation, or scoring records.
