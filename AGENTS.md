...
- Dated waivers that live in build tooling rather than in the published dataset stay out of `verifiedRecords()`, because they are not claims the site publishes. They still age under `evidenceFreshnessPolicy`: give each one a wall-clock test that fails past `maxAgeDays`, as `tests/source-health-reviewed-restrictions.test.ts` does for the reviewed source-health restrictions.
...
5. Context-only public activity, when a reviewed cask, stable installer release, or public repository exists: generated `src/data/gui-ecosystem-signals.ts` plus exact mappings and validation in `scripts/lib/gui-ecosystem-signal-mappings.mjs` and `scripts/lib/gui-ecosystem-signals.mjs`. Missing remains missing, every source-native unit remains separate, and none affects fit.
6. Workflow definitions and deterministic fit-band behavior: `src/lib/gui-fit.ts` and `src/lib/gui-types.ts`.
7. Freshness registration for products, logos, claims, sources, audits, exclusions, and activity snapshots: `src/lib/evidence-freshness.ts`.
8. GUI classification, provenance, SEO, sitemap, activity-sync, and freshness contracts in `tests/`.
9. GUI pages, methodology, navigation, search, sitemap, and `llms.txt` when public interpretation or routes change.
...
- Resolve release-review items only after reopening their official release notes. Record the human outcome, rationale, review date, and first-party evidence URL in the queue. Preserve that outcome only while the release-note digest is unchanged; changed notes must return to pending review. A `catalog-updated` outcome is incomplete until the relevant evidence, profile, propagation surfaces, and tests are committed in the same transaction.
...
- Context-only usage snapshots: `src/data/openrouter-attribution.ts`, `src/data/ecosystem-signals.ts`, and `src/data/gui-ecosystem-signals.ts`
...
- GUI activity sync and mappings: `scripts/sync-gui-ecosystem-signals.mjs`, `scripts/lib/gui-ecosystem-signals.mjs`, and `scripts/lib/gui-ecosystem-signal-mappings.mjs`
