...
- The dedicated usage refresh runs daily. It may update generated OpenRouter, Homebrew, npm, filtered GitHub release, VS Code Marketplace, Open VSX, JetBrains Marketplace, and GitHub repository context data, derived `/usage` and profile context, the static CSV, and the repository-only release-review queue. Release triage reads the separate reviewed harness watchlist rather than inferring coverage from downloadable assets; it must never reinterpret popularity or model triage as capability, quality, task success, or workflow fit.
