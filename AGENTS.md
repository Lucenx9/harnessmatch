...
7. Context-only OpenRouter attribution snapshots, when present: `src/data/openrouter-attribution.ts`. This file is generated by `npm run sync:openrouter` from canonical app pages and the authenticated 30-day coding dataset; do not hand-edit its values. Page totals and rolling-window metrics have separate dates and scopes. They are routing-channel observations, never capability evidence or scoring inputs.
