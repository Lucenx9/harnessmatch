...
const featureClaimSeedsByHarness = {
  ...
  codewhale: {
    mcp: documentedAt("2026-08-02", "MCP integration"),
    skills: documentedAt("2026-08-02", "Skills and subagents"),
    localModels: documentedAt("2026-08-02", "Provider registry"),
    subagents: documentedAt("2026-08-02", "Fleet runtime", "Subagent runtime"),
    headless: documentedAt("2026-08-02", "CodeWhale 0.9.3 source snapshot"),
    sandbox: configuredClaim(
      "optional",
      ["Sandbox threat model", "Authorization order"],
      "Optional macOS Seatbelt, opt-in Linux bubblewrap, or configured external OpenSandbox execution",
      "Unavailable local wrappers can fall back to host execution; Windows has no advertised local OS sandbox.",
      "2026-08-02",
    ),
    checkpoints: configuredClaim(
      "documented",
      ["CodeWhale 0.9.3 source snapshot"],
      "Turn-level restore in the interactive TUI",
      "Turn restore does not establish rollback of external side effects.",
      "2026-08-02",
    ),
  },
  openharness: {
    mcp: documentedAt("2026-08-02", "Skills, plugins, and MCP"),
    skills: documentedAt("2026-08-02", "Skills, plugins, and MCP", "OpenHarness 0.1.9 release notes"),
    localModels: documentedAt("2026-08-02", "Provider and subscription workflows"),
    subagents: documentedAt("2026-08-02", "OpenHarness 0.1.9 source snapshot"),
    headless: documentedAt("2026-08-02", "OpenHarness 0.1.9 source snapshot"),
    sandbox: configuredClaim(
      "optional",
      ["Optional sandbox adapter", "Docker sandbox backend", "Runtime settings"],
      "Optional sandbox-runtime OS wrapper or per-session Docker container",
      "Sandboxing and fail-closed behavior are disabled by default and depend on separately available runtimes.",
      "2026-08-02",
    ),
  },
  "grok-build": {
