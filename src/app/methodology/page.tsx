    ...
    <section className="section page-section">
      <div className="shell narrow-shell prose-page">
        ...
        <section className="prose-section" id="eligibility">
          ...
          <p>Only active products are eligible: dormant and archived products remain visible for research but are excluded from recommendations and benchmark rankings. OpenRouter and GitHub are discovery sources only: they can create a research candidate, but cannot establish a capability. When a canonical OpenRouter app page exists, its profile and the Usage page may show dated public-attribution context. The comparison uses OpenRouter’s authenticated coding-app dataset with explicit 1-day, 7-day, and 30-day UTC windows, rank, attributed tokens, requests, dataset version, and observation date. The most-used view follows source-native volume rank. The 7-day and 30-day trending views follow OpenRouter’s excess token-growth ordering against the average of the preceding three equal windows. The API publishes the resulting rank and current-window volume, not the excess amount, so HarnessMatch does not infer a growth percentage. Cumulative page totals, daily global rank, and models observed remain separately labeled because they have a different scope. OpenRouter explains that app visibility depends on identifying traffic through its <a href="https://openrouter.ai/docs/app-attribution" target="_blank" rel="noreferrer">app-attribution mechanism</a>; unattributed traffic, direct provider use, subscriptions, local execution, hidden apps, and private apps are therefore outside that view. OpenRouter also warns that upstream tokenizers differ, so attributed tokens are a routing-volume signal rather than standardized work or user count.</p>
          <p>The Usage page also refreshes exact artifact mappings from the <a href="https://formulae.brew.sh/docs/api/" target="_blank" rel="noreferrer">Homebrew analytics API</a>, the <a href="https://github.com/npm/download-counts" target="_blank" rel="noreferrer">npm downloads API</a>, explicitly filtered stable assets from the <a href="https://docs.github.com/en/rest/releases/releases" target="_blank" rel="noreferrer">GitHub releases API</a>, the public <a href="https://learn.microsoft.com/en-us/javascript/api/azure-devops-extension-api/eventcounts" target="_blank" rel="noreferrer">VS Code Marketplace install metric</a>, exact extensions from the <a href="https://github.com/eclipse-openvsx/openvsx" target="_blank" rel="noreferrer">Open VSX registry</a>, exact plugins from the <a href="https://plugins.jetbrains.com/docs/marketplace/api-reference.html" target="_blank" rel="noreferrer">JetBrains Marketplace API</a>, and the <a href="https://docs.github.com/en/rest/activity/starring" target="_blank" rel="noreferrer">GitHub stars API</a>. Homebrew 30d events and npm last-month downloads are rolling distribution-channel observations. Release assets, editor-marketplace records, and GitHub stars are cumulative. Release rows exclude drafts, prereleases, checksums, source archives, SDKs, GUI packages, and unrelated monorepo artifacts through reviewed name filters; repeated, automated, and multi-platform retrievals remain possible. GitHub rows preserve whether the audited repository contains full source, client source, or only support material. Downloads, installs, stars, tokens, and requests observe different populations and denominators, so they remain separate views and separate CSV rows. Missing coverage means no admitted mapping, not zero adoption. None of these signals changes recommendation order, evidence state, capability classification, or benchmark interpretation.</p>
          <p>The stable-release tracker is a separate factual view of the same GitHub API. It joins reviewed product-specific tag patterns to the canonical repository audit, excludes drafts, prereleases, and unrelated release trains, and publishes the latest tag, publication date, official release URL, repository scope, observation date, and count of matching stable releases in the trailing 90 days. It does not require downloadable assets and does not read language-model output. Release frequency is maintenance context rather than quality, adoption, or task-success evidence.</p>
