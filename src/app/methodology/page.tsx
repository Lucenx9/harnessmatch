...
const methodologyVersion = "2.7 / 2026-07-28";
    ...
    <section className="section page-section">
      <div className="shell narrow-shell prose-page">
        ...
        <section className="prose-section" id="eligibility">
          ...
          <p>Only active products are eligible: dormant and archived products remain visible for research but are excluded from recommendations and benchmark rankings. OpenRouter and GitHub are discovery sources only: they can create a research candidate, but cannot establish a capability. When a canonical OpenRouter app page exists, a profile may also show dated public-attribution context. The primary comparison uses OpenRouter’s authenticated coding-app dataset with one explicit rolling 30-day window, rank, attributed tokens, requests, dataset version, and observation date. Cumulative page totals, daily global rank, and models observed remain separately labeled because they have a different scope. OpenRouter explains that app visibility depends on identifying traffic through its <a href="https://openrouter.ai/docs/app-attribution" target="_blank" rel="noreferrer">app-attribution mechanism</a>; unattributed traffic, direct provider use, subscriptions, and local execution are therefore outside that view, while token accounting can vary across providers. First-party product telemetry may also be linked when its scope and observation date can be stated, but it remains descriptive context rather than evidence of task success or model quality. These signals never change recommendation order, evidence state, or capability classification.</p>
